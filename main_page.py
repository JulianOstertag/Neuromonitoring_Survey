...
@st.cache_data
def load_dataframe(f1,f2):
    raw_data = pd.read_excel(f1)
    question_names = pd.read_excel(f2)
    return raw_data, question_names

# Load the dataset
survey_filename = st.sidebar.file_uploader("Choose a CSV file", accept_multiple_files=False)
if survey_filename is None:
    survey_filename = "Z:\\MK klin EEG Projekte\\JULIAN\\Projects\\Neuromonitoring\\Survey\\Tableau\\Completed_Surveys.xlsx"
questions_filename = "Z:\\MK klin EEG Projekte\\JULIAN\\Projects\\Neuromonitoring\\Survey\\Tableau\\Question_Names.xlsx"
raw_data, question_names = load_dataframe(survey_filename,questions_filename)
